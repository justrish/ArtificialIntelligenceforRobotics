# -*- coding: utf-8 -*-
"""
Created on Mon Mar 27 00:32:43 2017
...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        steering_angle = random.gauss(motion[0],result.steering_noise)
        beta = (distance* math.tan (steering_angle))/result.length   
        if (math.fabs(beta) > 0.001 ):
            steering_radius = distance/beta
            cx = self.x - (math.sin(self.orientation)*steering_radius)
            cy = self.y + (math.cos(self.orientation)*steering_radius)
            result.orientation=math.fmod(self.orientation + beta ,2 * pi)
            result.x=cx + math.sin(result.orientation) * steering_radius
            result.y=cy - math.cos(result.orientation) * steering_radius
        else:
            result.orientation= math.fmod(self.orientation + beta,2 * pi)
            result.x=self.x + (distance * math.cos(result.orientation))
            result.y=self.y + (distance * math.sin(result.orientation))
